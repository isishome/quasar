...
&#60;/style&#62;
      ...
      <p>이름, 팀 명, 연락처 정보를 <em>props</em>로 새로 정의해 봅시다. 이 정보들은 <em>JSON</em> 객체 유형으로 그룹화해도 되지만 좀 더 명확하게 하기 위해 각각의 props를
        나누어 정의하도록
        하겠습니다.</p>
...
&#60;/template&#62;
      ...
      <p class="q-py-lg"></p>
      <p>멤버 카드를 컴포넌트로 분리하고 나니 코드가 좀 더 정돈된 느낌을 주지 않나요?
      </p>
      <p>위에서 설명하지 않은 코드도 보이는군요.(uid). <code>uid</code>는 Quasar에서 제공하는 일종의 고유 아이디 생성 유틸입니다. 사용법은 메서드 형태로 호출하게 되면
        <em>501e7ae1-7e6f-b923-3e84-4e946bff31a8</em>와 같은 문자열을 임의로 반환해 줍니다. 사용자에게는 보이지 않지만 각 컴포넌트마다 고유한 아이디를 갖게 된 셈입니다.
        이 고유 아이디는 계속되는 챕터에서 아주 유용하게 사용될 예정입니다.
      </p>
...
// 예: 501e7ae1-7e6f-b923-3e84-4e946bff31a8
      ...
      <p>컴포넌트 분리 작업이 간단하게 마무리 되었으니 새 멤버 카드를 추가하는 로직을 만들어 불까요?</p>
      <p>먼저 리스트 상단에 버튼을 추가 해봅시다.</p>
      <Code class="q-mb-lg" language="html">
        <textarea readonly>
&#60;!-- App.vue --&#62;
...
&#60;/template&#62;
      ...
      <p>추가 버튼이 멤버 리스트 우상단에 위치하도록 <em>div</em> 요소에 <code>row justify-end</code> 클래스를 사용했습니다.
        그 안에 <em>QBtn</em> 컴포넌트를 추가하고 click 이벤트에 멤버를 추가하는 메서드 핸들러를 정의해 봅시다.
      </p>
      <p>
        <code>addMember</code> 핸들러는 멤버 리스트 객체(반응형)의 맨 앞쪽에 빈 멤버 데이터를
        추가하는 간단한 로직으로 구성되어 있습니다(<em>mid</em>는 정의된 상태).
      </p>
